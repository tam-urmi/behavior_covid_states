...
sapply(datasets, function(d) sum(is.na(d$visit_1)))

sapply(datasets, function(d) table(d$visit_1, useNA = "always"))

get_behaviors <- function(dat) {
  wave <- first(dat$wave)
  data_wt <- list(svydesign(ids=~1, data=dat, weights=~weight_state),
                  svydesign(ids=~1, data=dat, weights=~weight), 
                  dat) 
  
  event0_state <- OrdinalTableNew("event", data =  data_wt, colGroup = c(2:9), digits = 2)
  event2_state <- OrdinalTableNew("event", data =  data_wt, colGroup = c(4,5,6), digits = 2)[,2]
  event3_state <- OrdinalTableNew("event", data =  data_wt, colGroup = c(7), digits = 2)[,2]
  event4_state <- OrdinalTableNew("event", data =  data_wt, colGroup = c(8,9), digits = 2)[,2]
  visit_tab <- OrdinalTableMulNew(visit_variables, data_wt, digits = 2)
  cov_beh_tab <- OrdinalTableMulNew(cov_beh_variables, data_wt, digits = 2, colGroup = 4)
  colnames(cov_beh_tab) <- substring(var_label(dat[cov_beh_variables]),100,200)
  
  
  event_tab <- cbind(event0_state[,2], event2_state, event3_state, event4_state)
  colnames(event_tab) <- c("Been in a room with someone outside of household in the past 24 hours", 
  ...
  out <- cbind(visit_tab, cov_beh_tab, event_tab) %>%
    as.data.frame() %>%
    mutate(state_code = row.names(visit_tab),
           Wave = wave,
           Start_Date = min(dat$StartDate),
           End_Date = max(dat$StartDate),
           N = event0_state[,4])  %>%
    left_join(dat %>% group_by(state_code) %>% summarize(Total_N=n())) %>%
    relocate(state_code)
  out[out$state_code=="National","Total_N"] <- nrow(dat)
  
  return(out)
}

cov_beh_variables <- paste0("cov_beh_", c(1,2,5))
colnames <- substring(var_label(datasets[[1]][cov_beh_variables]),100,200)
