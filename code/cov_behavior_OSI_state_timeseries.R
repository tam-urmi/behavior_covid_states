...
names(dat) = c("end_date", "state", "avoid_contact", "avoid_crowds", "wash_hands", "wear_mask", "work", "gym", "friend", "cafe", "doctor", "church", "transit", "room", "room_5_10", "room_11_50", "room_50_plus", "N", "new_ind")

osi_states

# MAE to compute the difference

osi_states_date_updated = osi_states %>%
  filter(RegionName %in% c("Massachusetts", "Virginia", "Louisiana", "Nebraska", "Alaska")) %>%
...
as_date(dat$start_date)

#("#8eace6", "#219ebc", "#023047", "#F6D55C", "#ED553B")
...
range(dat_MA$end_date)
range(osi_states_date_updated$date_true)
...
p1 = dat_MA %>%
  ...
  geom_line(data = osi_states_date_updated %>%
            ...
            linewidth = 0.75,
            show.legend = TRUE) +
  scale_color_manual(values = c("MA" = "darkblue")) +
...
p2 =  dat_VA %>%
  ...
  geom_line(data = osi_states_date_updated %>%
            ...
            linewidth = 0.75,
            show.legend = TRUE) +
  scale_color_manual(values = c("VA" = "darkblue")) +
...
p3 =  dat_LA %>%
  ...
  geom_line(data = osi_states_date_updated %>%
            ...
            linewidth = 0.75,
            show.legend = TRUE) +
  scale_color_manual(values = c("LA" = "darkblue")) +
...
p4 =  dat_NE %>%
  ...
  geom_line(data = osi_states_date_updated %>%
            ...
            linewidth = 0.75,
            show.legend = TRUE) +
  scale_color_manual(values = c("NE" = "darkblue")) +
...
p5 =  dat_AK %>%
  ...
  geom_line(data = osi_states_date_updated %>%
            ...
            linewidth = 0.75,
            show.legend = TRUE) +
  scale_color_manual(values = c("AK" = "darkblue")) +
...
ggsave(filename = "../covid_states_behavior1/figures/supplementary/OSI_avoid_contact_timeseries_v3.png", width = 8, height = 7, units = "in", bg = "white")
...
osi_states_date_updated %>%
  ggplot(aes(x = date_true, y = StringencyIndex_Average, group = state)) +
  geom_line(aes(color = state)) +
  geom_line(data = dat %>% filter(state %in% c("MA", "VA", "NE", "AK", "LA")),
            aes(x = as_date(start_date), y = avoid_contact, color = state)) +
  scale_color_brewer(palette = "Dark2") +
  labs(x = "Year", y = "% Adherence / OSI") +
...
dat_osi_nr %>%
  ...
  geom_line(aes(y = avoid_contact), alpha = 0.1) +
  geom_line(aes(y = StringencyIndex_Average), linetype = "dotted", alpha = 0.1) +
